...
class ExampleControllerClass {
  ...
  constructor(readonly $scope: myScope) {}
}
...
const componentConfig = jsxComponent({
  ...
  template: (ctrl, $, someOtherAvailableVariable) => (
    <div>
      ...
      <a ng-href={`img/${interpolation(ctrl.username, "uppercase")}.jpg`}>
        IMG2
      </a>
      ...
      {ngRepeat(
        ctrl.arr,
        (nutz, { $index, $odd, $even, $first, $last, $middle }) => (
          <div class="repeated">
            {$index}: {nutz}
          </div>
        ),
...
console.log(
  typeof componentConfig.template === "function"
