...
export function transformSource(sourceText: string, fileName = 'source.tsx'): string {
...
function isCalleeNamed(node: ts.CallExpression, name: string): boolean {
    const c = node.expression;
    if (ts.isIdentifier(c)) return c.text === name;
    if (ts.isPropertyAccessExpression(c)) return c.name.text === name;
    return false;
}

function tryLowerJsxTemplateCall(node: ts.CallExpression, sf: ts.SourceFile): Replacement | null {
    // jsxTemplate(fn)() 
    if (
...
function tryLowerJsxComponentCall(node: ts.CallExpression, sf: ts.SourceFile): Replacement[] | null {
    if (!isCalleeNamed(node, 'jsxComponent')) return null;
    // jsxComponent(options) | jsxComponent(app, name, options)
    const arg =
        node.arguments.length >= 3 && ts.isObjectLiteralExpression(node.arguments[2])
            ? node.arguments[2]
    ...
    const existingAs = arg.properties.find(
        (p): p is ts.PropertyAssignment =>
            ts.isPropertyAssignment(p) &&
...
function getArrowParamNames(fn: ts.Expression): string[] {
...
function lowerTemplateFn(fn: ts.Expression, sf: ts.SourceFile): string | null {
...
function unwrapJsxRoot(body: ts.ConciseBody): ts.JsxElement | ts.JsxSelfClosingElement | ts.JsxFragment | null {
    ...
    if (ts.isBlock(expr)) {
        ...
        if (rets.length !== 1 || !rets[0].expression) return null;
...
function jsxToHtml(
    node: ts.JsxElement | ts.JsxSelfClosingElement | ts.JsxFragment | ts.JsxText | ts.JsxExpression | ts.JsxChild,
    sf: ts.SourceFile,
): string | null {
...
function jsxChildToHtml(child: ts.JsxChild, sf: ts.SourceFile): string | null {
...
function expressionAsAttr(expr: ts.Expression, attrName: string, sf: ts.SourceFile): string | null {
    if (ts.isParenthesizedExpression(expr)) return expressionAsAttr(expr.expression, attrName, sf);
    ...
    if (ts.isObjectLiteralExpression(expr)) {
        if (attrName === 'style') return styleObjectToCss(expr, sf);
...
function expressionAsChild(expr: ts.Expression, sf: ts.SourceFile): string | null {
    if (ts.isParenthesizedExpression(expr)) return expressionAsChild(expr.expression, sf);

    if (ts.isCallExpression(expr)) {
        if (isCalleeNamed(expr, 'ngRepeat')) return lowerNgRepeat(expr, sf, false);
        if (isCalleeNamed(expr, 'ngRepeatObj')) return lowerNgRepeat(expr, sf, true);
        ...
        if (isCalleeNamed(expr, 'createElement')) return null;
    }
    ...
    if (ts.isArrayLiteralExpression(expr)) {
        ...
        if (expr.elements.length >= 1) {
            ...
            if (!first || ts.isSpreadElement(first)) return null;
...
function lowerNgRepeat(call: ts.CallExpression, sf: ts.SourceFile, isObj: boolean): string | null {
...
function injectAttribute(html: string, name: string, value: string): string | null {
...
function interpolationCallToChild(call: ts.CallExpression, sf: ts.SourceFile): string | null {
...
function interpolationCallToAttr(call: ts.CallExpression, sf: ts.SourceFile): string | null {
...
function arrowBodyToExpr(fn: ts.ArrowFunction | ts.FunctionExpression, sf: ts.SourceFile): string | null {
    ...
    return stripParensText(body as ts.Expression, sf);
}

function styleObjectToCss(obj: ts.ObjectLiteralExpression, sf: ts.SourceFile): string | null {
    const parts: string[] = [];
...
function objectToAngularExpr(obj: ts.ObjectLiteralExpression, sf: ts.SourceFile): string | null {
    const parts: string[] = [];
    for (const prop of obj.properties) {
        if (!ts.isPropertyAssignment(prop)) return null;
        const key = prop.name.getText(sf).replace(/^['"]|['"]$/g, '');
...
function templateToAngularString(
...
function isStaticPathExpr(expr: ts.Expression): boolean {
    if (ts.isIdentifier(expr)) return true;
    if (ts.isPropertyAccessExpression(expr)) return isStaticPathExpr(expr.expression);
    if (ts.isElementAccessExpression(expr)) {
        return (
            isStaticPathExpr(expr.expression) &&
...
function isStaticAngularExpr(expr: ts.Expression): boolean {
    if (isStaticPathExpr(expr)) return true;
    if (ts.isPrefixUnaryExpression(expr)) return isStaticAngularExpr(expr.operand);
    if (ts.isBinaryExpression(expr)) {
        return isStaticAngularExpr(expr.left) && isStaticAngularExpr(expr.right);
    }
    if (ts.isConditionalExpression(expr)) {
        return (
            isStaticAngularExpr(expr.condition) &&
    ...
    if (ts.isCallExpression(expr)) {
        ...
        if (!isStaticPathExpr(expr.expression) && !(
            ts.isPropertyAccessExpression(expr.expression) && isStaticPathExpr(expr.expression)
        )) {
    ...
    if (ts.isParenthesizedExpression(expr)) return isStaticAngularExpr(expr.expression);
...
function stripParensText(expr: ts.Expression | ts.Node, sf: ts.SourceFile): string {
    let n: ts.Node = expr;
