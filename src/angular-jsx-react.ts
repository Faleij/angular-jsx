...
import { jsxTemplate } from './angular-jsx';
import {signal, effect, Signal} from '@preact/signals-core';

const unscopedSignals: Signal[] = [];

let useStateHook;
export function useState(initialValue) {
    if (useStateHook) return useStateHook(initialValue);
  const sig = signal(initialValue);
  unscopedSignals.push(sig);
  const out: [any, (v: any) => void] & { signal: Signal } = [sig.value, (v) => { sig.value = v; }] as any;
  out.signal = sig;
  return out;
}

const unscopedEffects: [() => void | (() => void), any[]][] = [];

export function useEffect(setup: () => void | (() => void), dependencies: any[]) {
...
export function aJsxDirective(reactComponentFn) {
  ...
  const template = jsxTemplate(jsx);
  return () => {
    return {
      restrict: 'E',
      scope: {},
      controller: ['$scope', '$timeout', ($scope, $timeout) => {
          ...
          effect(() => {
            ...
            for (const signal of signals) signal.value;
            $timeout(digest);
          });
      }],
      template,
...
export function aJsxDirective2(reactComponentFn) {
  return () => {
    return {
      restrict: 'E',
      scope: {},
...
export function createElement(tagName, props?: Record<string, any>, ...children) {
    ...
    if (props) for (const [k, v] of Object.entries(props)) {
        if (!interceptProp(k, v, el)) el.setAttribute(k, renderValue(v, k));
    }
