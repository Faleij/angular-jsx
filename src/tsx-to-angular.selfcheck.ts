...
const out = transformSource(input, 'job-ad-like.tsx');
...
`;
const outReg = transformSource(registerForm, 'register.tsx');
checks.push(
    ['3-arg form lowers template', /template:\s*"/.test(outReg) && outReg.includes('ctrl.textSearch')],
    ['3-arg keeps app+name args', outReg.includes('jsxComponent(app, "jobAdCollection"')],
);
